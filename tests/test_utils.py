from src.utils import filter_airplanes, get_aeroplanes_by_altitude, print_aeroplanes, sort_airplanes
...
def test_print_aeroplanes_output(planes, capsys):
    ...
    assert (
        lines[0]
        == "1.Идентификатор: ab01 | Позывной: FLT-A | Страна регистрации: USA | Горизонтальная скорость (м/с)500.0 | Высота: 10000.0"
    )
    assert (
        lines[-1]
        == "5.Идентификатор: ab05 | Позывной: FLT-E | Страна регистрации: FR | Горизонтальная скорость (м/с)450.0 | Высота: 0.0"
    )
